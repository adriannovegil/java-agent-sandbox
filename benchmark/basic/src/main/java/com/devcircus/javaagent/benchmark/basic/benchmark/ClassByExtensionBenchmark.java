package com.devcircus.javaagent.benchmark.basic.benchmark;

import com.devcircus.javaagent.benchmark.basic.specimen.ExampleClass;
...
/**
 * <p>
 * This benchmark dynamically creates a subclass of {@link ExampleClass} which
 * overrides all methods to invoke the direct super class's implementation. The
 * benchmark furthermore creates an instance of this class since some code
 * generation frameworks rely on this property. Because this benchmark requires
 * the creation of a subclass, the JDK proxy is not included in this benchmark.
 * </p>
 * <p>
 * Note that this class defines all values that are accessed by benchmark
 * methods as instance fields. This way, the JIT compiler's capability of
 * constant folding is limited in order to produce more comparable test results.
 * </p>
 */
@State(Scope.Thread)
...
public class ClassByExtensionBenchmark {
    ...
    /**
     * Creates a new class loader. By using a fresh class loader for each
     * creation, we avoid name space issues. A class loader's creation is part
     * of the benchmark but since any test creates a class loader exactly once,
    ...
    /**
     * An implementation to be used by
     * {@link ClassByExtensionBenchmark#benchmarkByteBuddyWithProxyAndReusedDelegator()}.
     */
    private Implementation proxyInterceptor;

    /**
     * An implementation to be used by
     * {@link ClassByExtensionBenchmark#benchmarkByteBuddyWithAccessorAndReusedDelegator()}.
     */
    private Implementation accessInterceptor;

    /**
     * An implementation to be used by
     * {@link ClassByExtensionBenchmark#benchmarkByteBuddyWithPrefixAndReusedDelegator()}.
     */
    ...
    /**
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark creates proxy classes for the invocation of super methods which
     * requires the creation of auxiliary classes.
     *
    ...
    /**
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark also uses the annotation-based approach but creates delegation
     * methods which do not require the creation of additional classes. This
     * benchmark reuses a precomputed delegator.
     *
     * @return The created instance, in order to avoid JIT removal.
     ...
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark creates proxy classes for the invocation of super methods which
     * requires the creation of auxiliary classes. This benchmark uses a type
     ...
     * benchmark also uses the annotation-based approach but creates delegation
     * methods which do not require the creation of additional classes. This
     * benchmark reuses a precomputed delegator. This benchmark uses a type pool
     * to compare against usage of the reflection API.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     */
     ...
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark also uses the annotation-based approach but creates delegation
     * methods which do not require the creation of additional classes.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     */
     ...
     * benchmark also uses the annotation-based approach but creates delegation
     * methods which do not require the creation of additional classes. This
     * benchmark uses a type pool to compare against usage of the reflection
     * API.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
    ...
    /**
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark uses delegation but completes with a hard-coded super method
     * call.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     ...
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark uses delegation but completes with a hard-coded super method
     * call. This benchmark reuses a precomputed delegator.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     */
     ...
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark uses delegation but completes with a hard-coded super method
     * call. This benchmark uses a type pool to compare against usage of the
     * reflection API.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     ...
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark uses delegation but completes with a hard-coded super method
     * call. This benchmark reuses a precomputed delegator. This benchmark uses
     * a type pool to compare against usage of the reflection API.
     *
     * @return The created instance, in order to avoid JIT removal.
     * @throws Exception If the invocation causes an exception.
     */
    ...
    /**
     * Performs a benchmark of a class extension using Byte Buddy. This
     * benchmark uses a specialized interception strategy which is easier to
    ...
    /**
     * Instead of using the {@link net.bytebuddy.implementation.SuperMethodCall}
     * implementation, we are using a delegate in order to emulate the
     * interception approach of other instrumentation libraries. Otherwise, this
     * benchmark would be biased in favor of Byte Buddy.
     */
    public static class ByteBuddyProxyInterceptor {
         ...
         * @throws Exception As declared by
         * {@link java.util.concurrent.Callable}'s contract.
         */
        @RuntimeType
    ...
    /**
     * Instead of using the {@link net.bytebuddy.implementation.SuperMethodCall}
     * implementation, we are creating delegate methods that allow the
     * invocation of the original code.
     */
